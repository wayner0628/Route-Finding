...
def dfs(start, end):
    ...
    """----------------------Explanation-----------------------
    For dfs, I use a LIFO queue to store the nodes which may be visited later. 
    ...
    and the previous point of start point is itself, and the start point is visited. 
    Then keep the while-loop running until queue is empty or the end point is 
    discovered, each time pick a element from the queue, and use the dictionary 
    to find its neighbor nodes, if they are not yet be visited, put them into 
    the queue, and set their previous node as the current node pulling out from 
    the queue.
